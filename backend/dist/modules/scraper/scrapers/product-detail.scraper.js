...
class ProductDetailScraper {
    constructor(prisma) {
        this.prisma = prisma;
        this.logger = new common_1.Logger(ProductDetailScraper.name);
    }
            ...
            async requestHandler({ request, $ }) {
                ...
                for (const selector of reviewSelectors) {
                    $(selector).each((index, element) => {
                        ...
                        const author = $review.find('.reviewer-name').text().trim() ||
                            $review.find('[itemprop="author"]').text().trim() ||
                            'Anonymous';
                        const ratingText = $review.find('.rating').text().trim() ||
                            $review.find('[itemprop="ratingValue"]').text().trim() ||
                            '';
                        const text = $review.find('.review-text').text().trim() ||
                ...
                $('.specifications li, .specs li, .product-specs li').each((index, element) => {
                ...
                $('.specifications tr, .specs tr, .product-info tr').each((index, element) => {
